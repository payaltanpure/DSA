...
// The majority element appears more than `n/2` times, so it must occupy the
// middle position after sorting.
...
// * `O(log n)` (recursion stack) or `O(1)` extra space depending on the
// algorithm.
...
// Because the majority element appears more than half the time, it will survive
// all cancellations.
...
// The majority element occurs more than all other elements combined, so it
// cannot be completely canceled out.
...
// > Maintain a candidate and a vote count. Increase the count for the same
// element, decrease it for a different element, and reset the candidate when
// the count becomes zero. The majority element survives all cancellations.
...
// Inner Loop
// jnums[j]Compare with nums[i] = 2count02Equal112Equal221Not Equal231Not
// Equal241Not Equal252Equal362Equal4
// Final count = 4
