...
class Solution {
    public int[] plusOne(int[] digits) {
        // if all digits are 9 then that all digits becomes 0 becoz of digits[i]=0 and
        // then add 1 at fisrt postion it is done by lower side code
        for (int i = digits.length - 1; i >= 0; i--)
        // fetches last digit from digits array
        {
            if (digits[i] < 9) // if digit less than 9 take it as it is and add 1 to that digit means increment
                               // that digit by 1 and return digits array
            {
                digits[i]++;
                return digits;
            }
            // If digit = 9 → it becomes 0 and carries 1 to the previous digit so again
            // upper logic runs for previous digit
            digits[i] = 0;
        }
...
// [0, 0, 0, 0]
// Step 4: Put 1 at First Position
// result[0] = 1;
...
// LeetCode may provide arrays with hundreds of digits, which do not fit in
// long.
